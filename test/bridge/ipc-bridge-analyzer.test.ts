...
describe('extractModules', () => {
  ...
  it('finds defineIpcModule inside factory functions', () => {
    const factory = analyzeFixtureModules().find(
      (module) => module.name === 'factory',
    );
  ...
  it('classifies handleOnce and listenOnce as handlers and listeners', () => {
    const channelTypes = analyzeFixtureModules().find(
      (module) => module.name === 'channel-types',
    );
  ...
  it('serializes optional, rest, and async handler signatures', () => {
    const typed = analyzeFixtureModules().find(
      (module) => module.name === 'typed-args',
    );

    const optional = typed?.channels.find((channel) => channel.key === 'optional');
    const rest = typed?.channels.find((channel) => channel.key === 'rest');
    const asyncReturn = typed?.channels.find(
      (channel) => channel.key === 'asyncReturn',
    );
  ...
  it('collects emitted events from exported defineIpcEvents', () => {
    const status = analyzeFixtureModules().find(
      (module) => module.name === 'define-ipc-events',
    );
  ...
  it('warns when the same emitted event is declared twice', () => {
    const duplicate = analyzeFixtureModules().find(
      (module) => module.name === 'duplicate-events',
    );
  ...
  it('does not analyze ignored.test.ipc.ts even when present on disk', () => {
    ...
    expect(fileNames.some((fileName) => fileName.endsWith('ignored.test.ipc.ts'))).toBe(
      false,
    );
