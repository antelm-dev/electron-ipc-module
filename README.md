...
- **Electron:** two different claims, deliberately kept apart.
  ...
  - **No Electron runtime is launched in CI.** The unit suite mocks Electron (`test/setup.ts`) and the example job builds the preload bundle without starting the app, so the matrix shows that this package compiles and type-checks against each major — not that IPC behaves identically at runtime on all three. Structured-clone edge cases, `contextBridge` exposure, and sandboxed-preload behaviour are covered against the mock, not against a real process.
